//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.6 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2019.06.12 at 09:16:35 AM EDT 
//


package com.vmware.vim25;



/**
 * 
 */
@SuppressWarnings("all")
public class DvsApplyOperationFault
    extends DvsFault
{

    private final static DvsApplyOperationFaultFaultOnObject[] NULL_OBJECT_FAULT_ARRAY = new DvsApplyOperationFaultFaultOnObject[ 0 ] ;
    public DvsApplyOperationFaultFaultOnObject[] objectFault;

    public DvsApplyOperationFaultFaultOnObject[] getObjectFault() {
        if ((objectFault) == null) {
            return (NULL_OBJECT_FAULT_ARRAY);
        }
        return objectFault;
    }

    public void setObjectFault(DvsApplyOperationFaultFaultOnObject[] objectFault) {
        this.objectFault = objectFault;
    }

}
