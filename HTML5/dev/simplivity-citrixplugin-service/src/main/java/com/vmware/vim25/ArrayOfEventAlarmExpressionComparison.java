//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.6 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2019.06.12 at 09:16:35 AM EDT 
//


package com.vmware.vim25;



/**
 * 
 */
@SuppressWarnings("all")
public class ArrayOfEventAlarmExpressionComparison {

    private final static com.vmware.vim25.EventAlarmExpressionComparison[] NULL_EVENT_ALARM_EXPRESSION_COMPARISON_ARRAY = new com.vmware.vim25.EventAlarmExpressionComparison[ 0 ] ;
    public com.vmware.vim25.EventAlarmExpressionComparison[] EventAlarmExpressionComparison;

    public com.vmware.vim25.EventAlarmExpressionComparison[] getEventAlarmExpressionComparison() {
        if ((EventAlarmExpressionComparison) == null) {
            return (NULL_EVENT_ALARM_EXPRESSION_COMPARISON_ARRAY);
        }
        return EventAlarmExpressionComparison;
    }

    public void setEventAlarmExpressionComparison(com.vmware.vim25.EventAlarmExpressionComparison[] EventAlarmExpressionComparison) {
        this.EventAlarmExpressionComparison = EventAlarmExpressionComparison;
    }

}
