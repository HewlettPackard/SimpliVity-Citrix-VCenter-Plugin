...
"@
	...
	$domainPassword = ConvertTo-SecureString $adPassword -AsPlainText -Force
	$localCreds = New-Object PSCredential($adUsername, $domainPassword)
	$installOutpt = Invoke-VMScript -VM $vm -ScriptText $installscript -GuestCredential $localCreds -WarningAction Ignore
...
"@
	...
	Write-Host "Installation Script of $vmName : "$installscript
	start-sleep 30
	...
	Write-Host "$vmName : INSTALLATION OUTPUT "$installOutpt.ScriptOutput
	
	Write-Host "Checking whether the CWConnector is installed on '$vmName' or not"
	if($installOutpt.ScriptOutput)
	{
		Write-Host "Error occured !!!"
		Write-Error "CWCConnector didnt get installed in VM $vmName.So Clean the $vmName"
	}
	...
	#$cwcInstallConfirm = Invoke-vmScript -vm $vm -ScriptText $cmd -ScriptType powershell -GuestCredential $localCreds 
	...
	#	Write-Host "CWCConnector didnt get installed in VM $vmName.So Clean the $vmName"
	#	#cleanUp $vm
